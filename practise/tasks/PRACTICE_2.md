...
4. ##### Написать функцию, которая принимает массив чисел и возврашает новый массив, в котором числа больше нуля и меньше 10:
   ...
   * ![#FF0000](https://placehold.it/15/FF0000/000000?text=+) Ошибка  
   
            - Что должна вернуть функция?
            - Что сейчас возврашет функция и почему?
...
6. ##### Написать функцию, которая принимает массивы, в качестве аргументов(количество принимаемых аргументов не ограничено), и возврашает массив с наибольшим числом эелементов.
   ...
   * ![#FF0000](https://placehold.it/15/FF0000/000000?text=+) Ошибка 
            ...
            - Что должна вернуть функция?
            - Что означсет выражение: else if (num in arr) ...
            
   * ![#008000](https://placehold.it/15/008000/000000?text=+) Подсказка
            
            - Создай переменную, в которую, запишеш индекс массива с наибольшей длинной.
            - Выведи массив, использюя этот индекс.
...
7. ##### Написать функцию, которая возврашает минимальное значение элементов массива
   ...
   * ![#FF0000](https://placehold.it/15/FF0000/000000?text=+) Ошибка 
            ...
            - Во-первых: название функции - arrayMinValue
            - Какой результат выполнения функции и какой должен быть?
            - Найди ошибку: for(var i = 0; i < values.length; i++);
            - Что описывает это условие: if(values[i] < y) 
            - Каким может быть значение `y` при инициализации?
            
   * ![#008000](https://placehold.it/15/008000/000000?text=+) Подсказка
            
            - Возьми, в качестве значения `y`, значение первого элемента массива `values`
...
9. ##### Написать функцию, которая принимает 2 массива и возврашает значения, которые присутствуют в обоих массивах
   ...
   * ![#FF0000](https://placehold.it/15/FF0000/000000?text=+) Ошибка 
            ...
            - Попробуй подставить значения: 
              var arr11 = [1,2,3,4,5,6,7];
              var arr22 = [1,2,7,55,66,77,88,99, 4, 5];
              Результат будет прежним: [1, 2, 7]
              Почему?
            - Что означает выражение: a.length > 0 && b.length > 0
            
   * ![#008000](https://placehold.it/15/008000/000000?text=+) Подсказка
            
            - Опрдели массив с наибольшей длинной
            - Начни проход с массива с наибольшей длинной
            - Делая проверку, не забудь дополнительно проверить - существует ли `второй` массив с текущим индексом.
...
10. ##### Напиши функцию, которая принимает 3 массива и возврашает значения, которые не повторяются в массивах
    ...
    * ![#FF0000](https://placehold.it/15/FF0000/000000?text=+) Ошибка 
            ...
            - Это что: `for(var i = 0; i < a.length && i < b.length && i < c.length; i++)` ? 
            - `if(a[i] === b[i] && b[i] === c[i] in tmp) continue;` ?
   
    * ![#008000](https://placehold.it/15/008000/000000?text=+) Подсказка
             
             - Используй опыт предидущих задачь
...
11. ##### Написать функцию, которая принимает строку, и символ для поиска в строке. Функция должна выводить количество повторений символа в строке.
    ...
    * ![#008000](https://placehold.it/15/008000/000000?text=+) Подсказка
             ...
             - Из строки создай массив.
             - Создай переменную счетчик, которая будет обновлятся при каждом нахождении символа.
